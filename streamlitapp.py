...
if btn:
    ...
    if res[0]==1:
        p = round(res_proba[0][1], 3)
        res = f"hemophagocytic lymphohistiocytosis(probability:{p:.3f})"
    else:
        p = round(res_proba[0][0], 3)
        res = f"infectious mononucleosis(probability:{p:.3f})"
    st.markdown(f'''
